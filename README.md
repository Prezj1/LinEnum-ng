...
| `--hunt-users` | Cross-reference login-shell users from `/etc/passwd` against config/web/app dirs          |
...
# Add the (slower) login-shell user cross-reference hunt
./LinEnum-ng.sh --hunt-users
```
